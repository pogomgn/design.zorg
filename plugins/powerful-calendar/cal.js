(function ($) {
  ...
  function generateMonthDOM(currentDate) {
    ...
    for (var month in monthMapMy) {
      if (monthMapMy.hasOwnProperty(month)) {
  ...
  function generateMonthHeaderDOM(currentDate) {
    return (
      ...
      monthMapMy[currentDate.getMonth() + 1] +
      "</span>" +
  ...
  function generateWeekHeaderDOM(currentDate) {
    ...
    for (var weekDay in dayMapMy) {
      if (dayMapMy.hasOwnProperty(weekDay)) {
        str +=
          '<div class="day header" data-day="' +
          weekDay +
          '">' +
          dayMapMy[weekDay].substring(0, settings.weekDayLength) +
          "</div>";
      }
    }
